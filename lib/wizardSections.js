...
export function buildSectionErrorMap(validationState = {}, sections = FORM_SECTIONS) {
  const fieldErrors = validationState.errors || {};
  const sectionErrorMap = {};

  sections.forEach((section) => {
    sectionErrorMap[section.id] = section.fields.some((field) => Boolean(fieldErrors[field]));
  });

  sectionErrorMap['app-info'] = Boolean(
    sectionErrorMap['app-info']
      || validationState.avatarFileError
      || validationState.clientIdError
  );
  sectionErrorMap['app-details'] = Boolean(
    sectionErrorMap['app-details']
      || validationState.featuresError
      || validationState.screenshotsCountError
      || (validationState.screenshotFileErrors || []).some(Boolean)
  );
  sectionErrorMap['support-info'] = Boolean(
    sectionErrorMap['support-info']
