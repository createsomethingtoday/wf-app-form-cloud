...
function normalizeUploadedFileUrls(uploadedFile) {
  if (!uploadedFile) {
    return [];
  }
...
function resolveUploadedAssetUrls({ blobUrls = [], uploadedFiles }) {
  const avatarUrls = normalizeUploadedFileUrls(uploadedFiles?.avatar);
  const screenshotUrls = normalizeUploadedFileUrls(uploadedFiles?.screenshots);

  if (avatarUrls.length > 0 || screenshotUrls.length > 0) {
    return {
  ...
  return {
    avatarUrl: blobUrls[0] || '',
    screenshotUrls: blobUrls.slice(1),
  };
}
...
export function buildSubmissionWebhookData({
  ...
  const normalizedAppScopes = fields.appScopes !== undefined
    ...
    : undefined;
  const uploadedAssetUrls = resolveUploadedAssetUrls({ blobUrls, uploadedFiles });
  ...
  const rawData = {
    ...
    'App Avatar Image': uploadedAssetUrls.avatarUrl || (isUpdate ? undefined : ''),
    'App Screenshot 1': uploadedAssetUrls.screenshotUrls[0] || (isUpdate ? undefined : ''),
    'App Screenshot 2': uploadedAssetUrls.screenshotUrls[1] || (isUpdate ? undefined : ''),
    'App Screenshot 3': uploadedAssetUrls.screenshotUrls[2] || (isUpdate ? undefined : ''),
    'App Screenshot 4': uploadedAssetUrls.screenshotUrls[3] || (isUpdate ? undefined : ''),
    'App Screenshot 5': uploadedAssetUrls.screenshotUrls[4] || (isUpdate ? undefined : ''),
    submittedAt,
