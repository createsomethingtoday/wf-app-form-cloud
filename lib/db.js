...
async function getDatabase(runtime = {}) {
  ...
  if (!db) {
    console.error('SUBMISSIONS_DB missing diagnostics', {
      hasRuntimeDb: Boolean(runtime?.db),
      hasRuntimeEnv: Boolean(runtime?.env),
      runtimeEnvSummary: summarizeEnv(runtime?.env),
      resolvedEnvSummary: summarizeEnv(env),
    });
