...
describe('buildSubmissionWebhookData — New submissions', () => {
  ...
  it('preserves structured scope selections in the Airtable payload', () => {
    const { webhookData } = buildSubmissionWebhookData({
      fields: {
        ...baseFields,
        appScopes: [
    ...
    expect(JSON.parse(webhookData.payload.data['all-selected-scopes'])).toEqual([
      { id: 'assets', name: 'Assets', access: 'read-write' },
      { id: 'site-activity', name: 'Site activity', access: 'read' },
    ]);
  });
  ...
  it('uses explicit upload buckets when available instead of flat blob order', () => {
    const { webhookData } = buildSubmissionWebhookData({
      fields: baseFields,
      blobUrls: ['ss1.png', 'avatar.png'],
      uploadedFiles: {
    ...
    expect(d['App Avatar Image']).toBe('avatar.png');
    expect(d['App Screenshot 1']).toBe('ss1.png');
    expect(d['App Screenshot 5']).toBe('ss5.png');
  });
